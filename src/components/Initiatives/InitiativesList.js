...
const InitiativesList = (props) => {
	...
	const initiativesCollection =
		initiatives && initiatives.map((i) => <InitiativesCard key={i.id} theme={theme} id={i.id} />);
	...
	return (
		<InitiativesListStyled className={isClosed ? 'initiativesList' : 'initiativesList'}>
			<div className="content-wrapper">
				...
				{initiativesCollection.length > 0 ? (
					...
					"Aucune initiative crée pour ce département pour l'instant"
				)}
			</div>
